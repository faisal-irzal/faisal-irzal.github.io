...
This piece of information can further be used to select which venues or places match the criteria as we set above. The criteria #1 & #2 above can be translated to venue selection with the following category: 
Park | Forest | Lake | Market | Drugstore | Pharmacy | Grocery | Bus | Train | Tram|
The other criteria i.e. #3, #4 and #5 have already provided by the dataset.
